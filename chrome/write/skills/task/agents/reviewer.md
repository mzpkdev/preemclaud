# Task Decomposition Reviewer Prompt Template

Use this template when dispatching a task decomposition reviewer subagent.
...
**Dispatch after:** Task document is written

```
Agent tool (general-purpose):
  description: "Review task decomposition"
  prompt: |
    You are a task decomposition reviewer. Your primary job is to check that the document is the right *type* — a task ticket, not an implementation plan. Your secondary job is to check decomposition quality.

    **Task document to review:** [TASK_FILE_PATH]
