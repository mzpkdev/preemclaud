---
name: knowledge:docs
description: "Verify answers against live documentation when Claude's confidence is low. ALWAYS use this skill when: (1) you're unsure about an API signature, parameter names, return types, or method behavior; (2) the user asks about a specific version and you're not certain what changed in that version; (3) you spot a discrepancy between code already in context and what you'd expect the API to look like — wrong parameter names, unfamiliar import paths, deprecated patterns mixed with new ones; (4) the library is obscure, niche, or recently released and you have limited training data on it; (5) the user challenges your answer with 'are you sure?', 'check the docs', 'that doesn't look right', 'verify this', 'what do the docs say'; (6) the question is about internal/org-specific tools, APIs, or services that live in Confluence, Notion, or other internal wikis. Do NOT trigger for mainstream, well-known libraries (React, Express, Django, pandas, lodash, etc.) unless the user is asking about a specific version or you notice something in the code that contradicts your expectations. This skill prevents confident-but-wrong answers about APIs that have changed, libraries you don't know well, or internal tools you can't possibly know about."
user-invocable: true
