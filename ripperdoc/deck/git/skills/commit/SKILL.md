...
This skill delegates to a co-located agent in `${CLAUDE_SKILL_DIR}/workers/`. The agent runs git operations in its own
context window — diffs, planning, and execution never appear here.
...
If the response contains `<!-- COMMIT_DONE -->`, stop — the agent has finished (precondition failure, clean tree, or
commits executed).
...
1. If the response contains `<!-- COMMIT_DONE -->`, stop
