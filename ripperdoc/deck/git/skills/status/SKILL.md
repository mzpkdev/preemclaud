...
This skill delegates to a co-located agent in `${CLAUDE_SKILL_DIR}/workers/`. The agent runs in its own context window —
git diffs and gather output never appear here.
...
1. **Spawn** with `subagent_type: "Explore"` and the prompt below
