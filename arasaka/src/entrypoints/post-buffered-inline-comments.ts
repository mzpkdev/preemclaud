...
// upstream/src/entrypoints/post-buffered-inline-comments.ts
...
// upstream/src/github/api/client.ts
...
// upstream/src/entrypoints/post-buffered-inline-comments.ts
var BUFFER_PATH = "/tmp/inline-comments-buffer.jsonl";
var CLASSIFICATION_PROMPT = `You are classifying PR inline comments as either REAL code review feedback or TEST/PROBE calls.
...
Comments:
`;
async function classifyComments(bodies) {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) {
    console.log("ANTHROPIC_API_KEY not set \u2014 skipping classification, posting all unconfirmed comments");
    return null;
  }
  const prompt = CLASSIFICATION_PROMPT + bodies.map((b, i) => `${i + 1}. ${JSON.stringify(b)}`).join(`
`);
  try {
    ...
    if (!res.ok) {
      console.log(`Classification API returned ${res.status} \u2014 posting all unconfirmed comments`);
      return null;
    }
    ...
    if (!match) {
      console.log("Could not parse classification response \u2014 posting all unconfirmed comments");
      return null;
    }
    const parsed = JSON.parse(match[0]);
    if (!Array.isArray(parsed) || parsed.length !== bodies.length || !parsed.every((v) => typeof v === "boolean")) {
      console.log("Classification response shape mismatch \u2014 posting all unconfirmed comments");
      return null;
    }
    return parsed;
  } catch (e) {
    console.log(`Classification failed (${e instanceof Error ? e.message : String(e)}) \u2014 posting all unconfirmed comments`);
    return null;
  }
}
async function postComment(octokit, owner, repo, pull_number, headSha, c) {
  const params = {
    owner,
    repo,
    pull_number,
  ...
  try {
    ...
    console.log(`  failed ${c.path}:${c.line}: ${e instanceof Error ? e.message : String(e)}`);
    return false;
  }
}
async function main() {
...
`).filter(Boolean).map((line) => JSON.parse(line));
  ...
  if (!githubToken || !owner || !repo || !prNumber) {
    console.log("::warning::Missing GITHUB_TOKEN/REPO_OWNER/REPO_NAME/PR_NUMBER \u2014 cannot post buffered comments");
    return;
  }
  ...
  const toPost = verdicts === null ? candidates : candidates.filter((_, i) => verdicts[i] === true);
  const filtered = verdicts === null ? [] : candidates.filter((_, i) => verdicts[i] === false);
  if (filtered.length > 0) {
    console.log(`::warning::${filtered.length} buffered comment(s) classified as test/probe \u2014 NOT posted:`);
