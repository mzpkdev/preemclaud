...
python3 blackwall/bin/update.py --force
```

`decode` keeps the `.dat` files in place so `boot.py` continues to work while you edit. `encode` deletes the `.md` files after writing the `.dat` versions. After encoding, run `blackwall/bin/update.py --force` to sync changes to the plugin cache.
